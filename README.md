...
- **本地 OCR 是懒加载的**：即使启动时用的是接口引擎，之后从托盘菜单或设置窗口切回「本地」也能直接用
  （第一次用之前会自动初始化一次 OCR 环境）。真的缺 OCR 语言包时，切过去就会当场提示，不用等按 Alt+R 才发现。
- **每个服务的设置互相独立**：设置窗口里切换「识别服务」时，上一个服务没保存的改动会先存回它自己那一格，
  不会串到别的服务上。
- 配置每次写入都会在 `imgocr.log` 里留一行（引擎、各服务的模型、调用栈），方便排查"配置被谁改了"。
